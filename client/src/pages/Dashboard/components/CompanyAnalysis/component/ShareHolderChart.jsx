...
const ShareholdersEquityChart = ({ symbol = 'AAPL' }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:8000/api/financial/metrics/${symbol}`);
        if (!response.ok) {
      ...
      return (
        <div className="custom-tooltip">
          ...
          <p className="tooltip-value">
            Shareholders Equity: {formatValue(payload[0].value)}
          </p>
        </div>
      );
    }
  ...
  const calculateGrowth = () => {
    ...
    const oldest = data[0].shareholdersEquity;
    const newest = data[data.length - 1].shareholdersEquity;
    ...
    <div className="container">
      <div className="card">
        <div className="header">
          <h2 className="title">{symbol} Shareholders Equity</h2>
          <p className="subtitle">Annual shareholders equity by fiscal year</p>
        </div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <YAxis 
                ...
                width={90}
                label={{ value: 'Shareholders Equity (USD)', angle: -90, position: 'insideLeft', fill: '#94a3b8' }}
              />
              <Tooltip content={<CustomTooltip />} cursor={{ fill: 'rgba(168, 85, 247, 0.1)' }} />
              <Bar 
                dataKey="shareholdersEquity" 
                fill="#a855f7"
                radius={[8, 8, 0, 0]}
        ...
        <div className="stats-container">
          ...
          <div className="stat-card">
            <p className="stat-label">Latest Equity</p>
            <p className="stat-value">{data[data.length - 1] ? formatValue(data[data.length - 1].shareholdersEquity) : 'N/A'}</p>
          </div>
          {growth && (
            <div className="stat-card">
...
export default ShareholdersEquityChart;